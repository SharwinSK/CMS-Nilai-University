...
$postmortem_count = $postmortem_result->num_rows;
?>
            ...
            <div class="row">
                <div class="col-12">
                    <h2 class="section-title">
                        ...
                        <span class="stats-counter">Total: <?= $proposal_count ?> events</span>
                    </h2>
                        ...
                        <table class="table table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 10%;">Advisor Feedback</th>
                                    <th style="width: 10%;">Coordinator Feedback</th>
                            ...
                            <tbody id="proposalTableBody">
                                <?php foreach ($feedback_by_event as $proposal): ?>
                                    ...
                                    // Change the status display text
                                    if ($status === 'Approved by Advisor (Pending Coordinator Review)') {
                                        $status_display = 'Pending Coordinator Review';
                                    } else {
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <span class="status-badge <?= $badge_class ?>"
                                                title="<?= $status_display ?>"><?= $status_display ?></span>
                                        </td>
                                        ...
                                        <td>
                                            <div class="action-buttons">
                                                ...
                                                if ($postmortem_exists) {
                                                    ...
                                                    echo '<button class="action-btn btn-edit" onclick="editProposal(\'' . $proposal['Ev_ID'] . '\')" title="Edit Proposal"><i class="fas fa-edit"></i></button>';
                                                    echo '<button class="action-btn btn-view" onclick="viewProposal(\'' . $proposal['Ev_ID'] . '\')" title="View Proposal"><i class="fas fa-eye"></i></button>';
                                                } elseif ($status === 'Rejected by Advisor' || $status === 'Rejected by Coordinator') {
                                                    echo '<button class="action-btn btn-modify" onclick="modifyProposal(\'' . $proposal['Ev_ID'] . '\')" title="Modify Proposal"><i class="fas fa-wrench"></i></button>';
                                                    echo '<button class="action-btn btn-delete" onclick="deleteProposal(\'' . $proposal['Ev_ID'] . '\')" title="Delete Proposal"><i class="fas fa-trash"></i></button>';
                                                } elseif ($status === 'Approved by Coordinator') {
                                                    echo '<button class="action-btn btn-view" onclick="viewProposal(\'' . $proposal['Ev_ID'] . '\')" title="View Proposal"><i class="fas fa-eye"></i></button>';
                                                    echo '<button class="action-btn btn-postmortem" onclick="createPostmortem(\'' . $proposal['Ev_ID'] . '\')" title="Create Post Event Report"><i class="fas fa-plus"></i></button>';
                                                } else {
                                                    echo '<span class="text-muted">N/A</span>';
                                                }
                                                ?>
            ...
            <div class="row">
                <div class="col-12">
                    <h2 class="section-title">
                        ...
                        <span class="stats-counter">Total: <?= $postmortem_count ?> events</span>
                    </h2>
                    <div class="table-responsive">
                        <table class="table table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 12%;">Coordinator Feedback</th>
                                    <th style="width: 12%;">Export</th>
                            ...
                            <tbody id="postEventTableBody">
                                ...
                                $postmortem_stmt->execute();
                                $postmortem_result = $postmortem_stmt->get_result();
                                while ($report = $postmortem_result->fetch_assoc()):
                                    ?>
                                    <?php
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <button class="action-btn btn-export"
                                                onclick="exportPostEvent('<?= $report['Rep_ID'] ?>')" title="Export PDF">
                                                <i class="fas fa-download"></i>
                                            </button>
                                        </td>
                                        <td>
                                            <div class="action-buttons">
                                                ...
                                                if ($status === 'Postmortem Pending Review') {
                                                    echo '<button class="action-btn btn-edit" onclick="editPostEvent(\'' . $report['Rep_ID'] . '\')" title="Edit Post Event Report"><i class="fas fa-edit"></i></button>';
                                                    echo '<button class="action-btn btn-view" onclick="viewPostEvent(\'' . $report['Rep_ID'] . '\')" title="View Post Event Report"><i class="fas fa-eye"></i></button>';
                                                } elseif ($status === 'Postmortem Rejected') {
                                                    echo '<button class="action-btn btn-modify" onclick="modifyPostEvent(\'' . $report['Rep_ID'] . '\')" title="Modify Post Event Report"><i class="fas fa-wrench"></i></button>';
                                                } else {
                                                    echo '<span class="text-muted">N/A</span>';
                                                }
                                                ?>
